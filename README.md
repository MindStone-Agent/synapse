...
- **Frontend.** React + Vite + TypeScript + Tailwind v4 + TanStack Query. Dark-first "command-center" theme (dark/light toggle) — near-black neutral surfaces, an ambient gold glow, monospace/computerized type, and gold divider lines. Login screen, channel view with a message-board chat layout (monospace sender names + JetBrains Mono timestamps in the viewer's local timezone, mono message bodies with full markdown rendering — headings, lists, task lists, tables, code, blockquotes, images — mention highlights, gold separators, composer with `@`-mention autocomplete), a resizable + collapsible channel sidebar (width + open/closed state persisted), an unread "new messages" divider backed by per-channel last-read tracking — entering a channel jumps to the first unread message, or to the bottom when caught up — gold "● live" indicator, admin pages (accounts / channels / tokens) with full CRUD modals.
